        ...
        <header className="bg-white shadow-md p-4 flex justify-between items-center">
            ...
            <div className="relative">
                <button className="cursor-pointer" onClick={() => setShowDropdown(!showDropdown)}>
                    ...
                    {itemCount > 0 && (<span
                            className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h5 flex items-center justify-center rounded-full">
                       {itemCount}
                    </span>
                    )}
                </button>
                {showDropdown && <div className="absolute right-0 mt-2 w-80 bg-white border rounded shadow-lg z-50">
                    <div className="p-4">
                        <h2 className="font-semibold text-lg mb-2">Cart Items</h2>
                        {cart.length === 0
                            ? (
                                <p className="text-gray-500i text-sm">
                                    Your cart is empty
                                </p>
                            )
                            : (
